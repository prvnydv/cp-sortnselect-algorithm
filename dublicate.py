...
def img_hash(file_id, hash_size, drive):
    return imagehash.phash(read_file_content_gdrive(drive, file_id),hash_size=hash_size)
    ...
    for url in url_array:
        if ('file' not in hashes.columns) or (url not in list(hashes['file'])):
            try:                     
                ...
                result = {'file': url,'hash':img_hash(url,hash_size, drive)}
...
def remove_similar_from_dir(url_array,hash_size,threshold, job_uid, drive):
    hashes_16_lag = get_hashes(url_array,hash_size, job_uid, drive)
    ...
    print(f"Distances between image hashes :: {distances}")
